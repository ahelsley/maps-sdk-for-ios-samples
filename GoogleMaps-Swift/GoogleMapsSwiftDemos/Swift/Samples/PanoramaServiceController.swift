...
final class PanoramaServiceController: UIViewController {
  ...
  override func viewDidAppear(_ animated: Bool) {
    if panoService == nil {
      ...
      statusLabel.text = "Service initialized \(panoService != nil ? "\(panoService!)" : "nil")"
    }

    if let ps = panoService {
      requestCount += 1
      ps.requestPanoramaNearCoordinate(
        markerLocation,
        radius: 300 /*meters*/,
        source: .outside
      ) {
        ...
        self.statusLabel.text = "Callback invoked \(self.responseCount) time(s) out of \(self.requestCount) requests."

        if let pano = pano {
          self.result.text = "id: \(pano.panoramaID)\n#neighbors: \(pano.links.count)\ncoordinate: \(pano.coordinate)\ndescription: \(pano.description)\ndebugDescription: \(pano.debugDescription)"
        } else {
